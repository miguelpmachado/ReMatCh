...
def alleleCalling(bamSortedPath, referencePath, sequenceNames, gatkPath, sampleID, qualityThreshold, coverageThreshold, multipleAlleles, sequenceMedObject, sequenceAndIndex):
	...
	os.system("bcftools filter --include 'QUAL>=" + qualityThreshold + " && FORMAT/DP>=" + coverageThreshold + " && INFO/DP/FORMAT/DV>=" + multipleAlleles + "' --output-type v --output " + bamSortedPath + "_filtered.vcf " + bamSortedPath + ".vcf")
