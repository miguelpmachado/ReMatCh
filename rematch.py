...
def main():
	...
	requiredNamed.add_argument('-cov', '--min-coverage', nargs='?', metavar=('N'), type=int, help='Minimum coverage depth required for base calling and SNP calling.', default = 10, required=False)
	requiredNamed.add_argument('-qual', '--min-quality', nargs='?', metavar=('N'), type=int, help='Minimum mapping quality for SNP calling', default = 10, required=False)
	requiredNamed.add_argument('-mul', '--multiple', nargs='?', metavar=('0.0 - 1.0'), type=float, help='Minimum read mapping frequency to be considered as ', default = 0.75, required=False)
	requiredNamed.add_argument('-threads', nargs='?', metavar=('N'), type=int, help='Number of threads used to run bowtie2', required=False, default= 1)
	requiredNamed.add_argument('-tax', nargs='?', metavar=('Streptococcus pneumoniae'), type=str, help='Name taxon to download sequences. Results will be stored in /path/to/idenfifiersList.txt', required=False)
	requiredNamed.add_argument('-xtraSeq', nargs='?', type=int, help='For trimming extra sequence lenght 5\' and 3\' ', required=False, default = 0)
	requiredNamed.add_argument('-bowtieBuild', help='Run build bowtie', action='store_true')
	requiredNamed.add_argument('-clean', help='Clean intermediate files produced by the application (.bam, .vcf, index files, coverage file)', action='store_true')
	requiredNamed.add_argument('-rmFastq', help='Remove fastq files after the analysis', action='store_true')
	requiredNamed.add_argument('-allplat', help='Use all platforms. By default, only Illumina runs are used', action='store_true')

	args = requiredNamed.parse_args()
